...
export default function Flower() {
    ...
    const [animationsChanged, setAnimationsChanged] = useState(false);
    let animationInstance = null;
    const initialPaths = {}; // Store initial paths
    ...
    useEffect(() => {
        ...
        const svgContainer =  document.getElementById("svgContainer");
         ...
         // Store initial paths
         const pathsToStore = ['outerTop', 'innerTop', 'outerLeft', 'innerLeft', 'outerRight', 'innerRight', 'outerBottom', 'innerBottom'];
         pathsToStore.forEach(pathId => {
             const path = document.getElementById(pathId);
             if (path) {
                 initialPaths[pathId] = path.getAttribute("d");
             }
         });
        ...
        function resetAnimation() {
            ...
            const pathsToReset = ['outerTop', 'innerTop', 'outerLeft', 'innerLeft', 'outerRight', 'innerRight', 'outerBottom', 'innerBottom'];
            pathsToReset.forEach(pathId => {
                const path = document.getElementById(pathId);
                if (path && initialPaths[pathId]) {
                    KUTE.to(path, { path: initialPaths[pathId] }).start();
                }  });
